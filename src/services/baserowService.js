...
export async function getGuest(phoneNumber){
    try{
        const { data } = await baserow_api.get(`/rows/table/${table_id}/?user_field_names=true&filter__field_${col_phoneNumber_ID}__contains=${phoneNumber}`)
...
export async function confirmPresence(guestID, confirmations){
    try{
        ...
        const response = await baserow_api.patch(`/rows/table/${table_id}/${guestID}/?user_field_names=true`, payload)
